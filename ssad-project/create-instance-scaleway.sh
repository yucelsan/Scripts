...
# --- Début ---
echo -e "\n$ROCKET Démarrage du script de création - $(date)"

VOLUME_NAME="${INSTANCE_NAME}-volume-$(date +%Y%m%d-%H%M%S)"

# --- Création du volume ---
echo -e "\n$DISK Création du volume $VOLUME_NAME (Block Storage 5K)"
...
if [[ -z "$VOLUME_ID" || "$VOLUME_ID" == "null" ]]; then
  echo "$CROSS ERREUR : le volume n'a pas été créé. Abandon."
  exit 1
fi
...
echo -e "\n$INFO Création de l'instance $INSTANCE_NAME avec volume attaché"
...
# echo "$FOLDER Copie et déploiement docker-compose frontend/backend..."
